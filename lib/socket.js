...
module.exports = class UDXSocket extends events.EventEmitter {
  ...
  _onclose () {
    console.info(this.counter, 'on close was called')
    this.emit('close')
  }
  ...
  _closeMaybe () {
    if (!this._closed && this._inited && this.closing && this.idle) {
      ...
      console.warn(this.counter, `Did not close immediately--setting hacky timeout.\nCurrent this.streams.size: ${this.streams.size}--inited:${this._inited}`)
      setTimeout(() => this._closeMaybe(), 5000)
    }
