...
module.exports = class UDXSocket extends events.EventEmitter {
  ...
  addMembership(group, iface_address = "") {
    if (!this._inited) throw new Error('Socket not active')
    return binding.udx_napi_socket_set_membership(this._handle, group, iface_address, true)
  }

  dropMembership(group, iface_address = "") {
    if (!this._inited) throw new Error('Socket not active')
    return binding.udx_napi_socket_set_membership(this._handle, group, iface_address, false)
  }
